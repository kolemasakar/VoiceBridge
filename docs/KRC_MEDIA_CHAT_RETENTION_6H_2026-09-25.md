...
## Existing-module audit and exact text correction
The existing cloud already contains pagination in media_client_http.ts (liveSegments), public_cobalt_media.ts, managed_media_service.ts and the YouTube Gemini engine. The new collector is only a reusable read-only client helper, not a replacement for these four platform pipelines. The Gemini direct provider calls chunkTranscriptWords([], transcriptText); its fallback chunker previously trimmed each individual 1600-character chunk, silently losing boundary spaces. Candidate code commit dabed98 changes chunk text to preserve boundary whitespace and adds a long-text round-trip regression. Targeted TypeScript build and 12 tests passed on isolated checkout. The current private Plugin is not modified. For late-return policy, first use existing lookup/status/segments; request fresh consent only if reprocessing is needed and the prior result cannot be recovered.
