...
import { createInternalDbExportHttpHandler } from "./internal_db_export_http.js";
...
export function createManagedVoiceBridgeServer(config: AppConfig) {
  ...
  const internalDbExport = createInternalDbExportHttpHandler(config);
  ...
  server.on("request", async (request, response) => {
    if (await internalDbExport.handle(request, response)) return;
