...
test("Gemini Live setup and 48 kHz PCM resampling preserve the STT contract", async () => {
  ...
  try {
    ...
    const frame = constantPcm16(960, 3000);
    for (let index = 0; index < 5; index += 1) {
      assert.equal(connection.sendAudio(frame), true);
    }
    ...
    assert.equal(providerAudio.length, 1);
    assert.equal(providerAudio[0]?.byteLength, 3200);
    assert.equal(providerMimeTypes[0], `audio/pcm;rate=${GEMINI_STT_SAMPLE_RATE_HZ}`);
    const audio = providerAudio[0]!;
    for (let offset = 0; offset < audio.byteLength; offset += 2) {
      assert.equal(audio.readInt16LE(offset), 3000);
    }
